...
class PMCPnPAnnealingIterator(torch.nn.Module):
    def __init__(self, gamma, sigmas, alphas=torch.ones(1000), max_iter=1e7, deterministic = False, Himbert = False):
        super().__init__()
        self.gamma = gamma
        self.alphas = alphas
        self.noise_std = np.sqrt(2 * gamma)
...
class PMCPnPAnnealing(dinv.sampling.MonteCarlo):
    def __init__(
        self, prior, data_fidelity, sigmas, gamma, alphas, max_iter=1e3, thinning=1, burnin_ratio=0.4, clip=(-1, 2), verbose=True, deterministic = False, Himbert = False,
    ):
        # generate an iterator
        iterator = PMCPnPAnnealingIterator(gamma=gamma, sigmas=sigmas, alphas=alphas, max_iter=max_iter,  deterministic = deterministic, Himbert = Himbert)
        # set the params of the base class
        super().__init__(
            iterator, prior, data_fidelity, max_iter=max_iter, thinning=thinning, burnin_ratio=burnin_ratio, clip=clip, verbose=verbose,
        )
